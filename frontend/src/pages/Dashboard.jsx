...
import { Plus, Calendar, Search, CheckCircle, CalendarClock, List } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [activeFilter, setActiveFilter] = useState('all'); // State for filtering: 'all', 'upcoming', 'completed'
    ...
    <div className="bg-[#0A1931] min-h-screen text-white p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <div className="shrink-0"><h1 className="text-3xl font-bold text-white">My Events</h1></div>
          <div className="relative w-full sm:w-auto sm:flex-grow sm:max-w-md">
            <input type="text" placeholder="Search events by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-[#1F294A] border border-gray-600 rounded-lg py-2 pl-10 pr-4 w-full focus:outline-none focus:ring-2 focus:ring-amber-500" />
          ...
          <div className="flex gap-2 sm:gap-4 items-center shrink-0">
            ...
            <button onClick={handleLogout} className="bg-red-600/80 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-600 transition-colors">Log Out</button>
          </div>
        </header>
